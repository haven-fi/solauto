use marginfi_sdk::generated::{
    ...
    instructions::{
        LendingAccountBorrowCpi, LendingAccountBorrowCpiAccounts,
        LendingAccountBorrowInstructionArgs, LendingAccountDeposit, LendingAccountDepositCpi,
        LendingAccountDepositCpiAccounts, LendingAccountDepositInstructionArgs,
        LendingAccountRepayCpi, LendingAccountRepayCpiAccounts, LendingAccountRepayInstructionArgs,
        LendingAccountWithdraw, LendingAccountWithdrawCpi, LendingAccountWithdrawCpiAccounts,
        LendingAccountWithdrawInstructionArgs, MarginfiAccountInitializeCpi,
...
impl<'a> MarginfiClient<'a> {
    pub fn initialize<'b>(
        ...
        let marginfi_account_owner =
            solauto_utils::get_owner(solauto_position, ctx.accounts.signer);
        ...
        if marginfi_account_owner.key == solauto_position.account_info.key {
            ...
            let transformed: Vec<&[u8]> = position_seeds.iter().map(|v| v.as_slice()).collect();
...
impl<'a> LendingProtocolClient<'a> for MarginfiClient<'a> {
    ...
    fn deposit<'b>(
        ...
        if authority.key == std_accounts.solauto_position.account_info.key {
            let position_seeds =
                solauto_utils::get_solauto_position_seeds(&std_accounts.solauto_position);
            let transformed: Vec<&[u8]> = position_seeds.iter().map(|v| v.as_slice()).collect();
            cpi.invoke_signed(&[transformed.as_slice()])
        } else {
            cpi.invoke()
