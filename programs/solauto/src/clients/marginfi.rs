...
const CONF_INTERVAL_MULTIPLE: f64 = 2.12;
const STD_DEV_MULTIPLE: f64 = 1.96;
...
impl<'a> MarginfiClient<'a> {
    ...
    pub fn get_max_ltv_and_liq_threshold(
        ...
        let max_ltv = i80f48_to_f64(
            I80F48::from_le_bytes(supply_bank.data.config.asset_weight_init.value)
        ).div(
            i80f48_to_f64(I80F48::from_le_bytes(debt_bank.data.config.liability_weight_init.value))
        );

        let liq_threshold = i80f48_to_f64(
            I80F48::from_le_bytes(supply_bank.data.config.asset_weight_maint.value)
        ).div(
            i80f48_to_f64(I80F48::from_le_bytes(debt_bank.data.config.liability_weight_maint.value))
        );
    ...
    pub fn get_supply_token_usage(
        ...
        let market_price = MarginfiClient::load_price(
            &bank,
            price_oracle,
            price_type,
            TokenType::Supply
    ...
    pub fn get_debt_token_usage(
        ...
        let amount_can_be_used = min(
            bank.data.config.borrow_limit.saturating_sub(i80f48_to_u64(total_borrows)),
            i80f48_to_u64(base_unit_supply_available)
        );

        let borrow_fee_bps = i80f48_to_f64(
    ...
    pub fn load_price(
        ...
        match bank.data.config.oracle_setup {
            ...
            OracleSetup::PythPushOracle => {
                ...
                let (price, conf, exponent) = if price_type == PriceType::Ema {
                    (
                        price_feed.price_message.ema_price,
                        price_feed.price_message.ema_conf,
                        price_feed.price_message.exponent,
                    )
                } else {
                    (
                        price_feed.price_message.price,
                        price_feed.price_message.conf,
                        price_feed.price_message.exponent,
                    )
                };
                ...
                Ok(
                    with_conf_interval(
                        derive_value(price, exponent), 
                        derive_value(conf as i64, exponent).mul(CONF_INTERVAL_MULTIPLE),
                        token_type
                    )
                )
            }
...
impl<'a> LendingProtocolClient<'a> for MarginfiClient<'a> {
    ...
    fn withdraw<'c>(
        ...
        let marginfi_account_data = DeserializedAccount::<MarginfiAccount>
            ::zerocopy(Some(self.marginfi_account))?
            .unwrap().data;

        let active_balances = marginfi_account_data.lending_account.balances
            .iter()
        ...
        if !withdrawing_all && active_balances.len() == 1 {
            ...
            let supply_balance = i80f48_to_u64(
                asset_shares.mul(
    ...
    fn borrow<'c>(
        ...
        let mut remaining_accounts: Vec<AccountMetaFlags> = Vec::<AccountMetaFlags>::with_capacity(
            4
        );
