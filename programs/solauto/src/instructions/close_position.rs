...
pub fn close_position_ta<'a>(
    ...
    let position_ta_data = solauto_utils
        ::safe_unpack_token_account(Some(position_ta))?
        .unwrap().data;

    if position_ta_data.amount > 0 && position_ta_data.mint != WSOL_MINT {
        solana_utils::spl_token_transfer(ctx.accounts.token_program, SplTokenTransferArgs {
            source: position_ta,
