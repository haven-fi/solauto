...
fn update_token_prices<'a>(
    ...
    std_accounts.solauto_position.data.state.supply.update_market_price(supply_price);
    std_accounts.solauto_position.data.state.debt.update_market_price(debt_price);
...
fn rebalance<'a>(
    ...
    check!(std_accounts.authority_referral_state.is_some(), SolautoError::IncorrectAccounts);
    check!(args.flash_loan_fee_bps.unwrap_or(0) <= 150, SolautoError::IncorrectInstructions);

    let fees_bps = SolautoFeesBps::from(
        std_accounts.authority_referral_state.as_ref().unwrap().data.referred_by_state !=
            Pubkey::default(),
