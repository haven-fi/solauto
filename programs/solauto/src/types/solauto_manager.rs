...
impl<'a> SolautoManager<'a> {
    ...
    fn withdraw(
        ...
        let base_unit_amount = match amount {
            TokenBalanceAmount::All => {
                self.std_accounts.solauto_position.data.state.supply.amount_used.base_unit
            }
    ...
    fn repay(&mut self, amount: TokenBalanceAmount) -> ProgramResult {
        let base_unit_amount = match amount {
            TokenBalanceAmount::All => {
                self.std_accounts.solauto_position.data.state.debt.amount_used.base_unit
            }
            TokenBalanceAmount::Some(num) => num,
        };
    ...
    fn update_usage(&mut self, base_unit_amount: i64, token_type: TokenType) {
        if
            !self.std_accounts.solauto_position.data.self_managed.val ||
    ...
    pub fn begin_rebalance(&mut self, rebalance_args: &RebalanceSettings) -> ProgramResult {
        ...
        if amount_to_dca_in.is_some() {
            if self.std_accounts.solauto_position.data.position.dca.token_type == TokenType::Supply {
        ...
        let rebalance_type = self.std_accounts.solauto_position.data.rebalance.rebalance_type;
        if rebalance_type == SolautoRebalanceType::DoubleRebalanceWithFL {
            validate_debt_adjustment(
                ...
                self.std_accounts.solauto_position.data.rebalance.flash_loan_amount,
                debt_adjustment_usd,
                Some(&rebalance_type)
            )?;
            return Ok(());
    ...
    pub fn finish_rebalance(&mut self, rebalance_args: &RebalanceSettings) -> ProgramResult {
        ...
        if !self.std_accounts.solauto_position.data.self_managed.val {
            let dca_in_base_unit =
                self.std_accounts.solauto_position.data.position.dca.dca_in_base_unit;
            let dca_token_type = self.std_accounts.solauto_position.data.position.dca.token_type;
        ...
        if flash_loan_amount > 0 {
            let flash_loan_fee_bps = if boosting {
                self.std_accounts.solauto_position.data.state.supply.flash_loan_fee_bps
            } else {
                self.std_accounts.solauto_position.data.state.debt.flash_loan_fee_bps
            };
    ...
    pub fn refresh_position(
        ...
        solauto_position.state.supply.update_market_price(updated_data.supply.market_price);
        ...
        solauto_position.state.debt.update_market_price(updated_data.debt.market_price);
        ...
        if solauto_position.position.setting_params.automation.is_active() {
            ...
            if automation.eligible_for_next_period(clock.unix_timestamp as u64) {
                ...
                solauto_position.position.setting_params.boost_to_bps =
                    automation.updated_amount_from_automation(
                        solauto_position.position.setting_params.boost_to_bps,
                ...
                if new_periods_passed == automation.target_periods {
                    ...
                    solauto_position.position.setting_params.automation.periods_passed =
                        new_periods_passed;
                }
            }
        }
