...
impl<'a, 'b> SolautoManager<'a, 'b> {
    pub fn from(
        ...
        let solauto_fees_bps =
            solauto_utils::SolautoFeesBps::from(!&std_accounts.referred_by_supply_ta.is_none());
        Ok(Self {
            client,
            obligation_position,
            accounts,
    ...
    pub fn protocol_interaction(&mut self, action: SolautoAction) -> ProgramResult {
        match action {
            ...
            SolautoAction::Borrow(base_unit_amount) => {
                self.borrow(
                    base_unit_amount,
                    self.accounts.debt.as_ref().unwrap().source_ta,
                )?;
            }
    ...
    fn finish_rebalance(&mut self) -> ProgramResult {
        ...
        let debt_position_ta =
            TokenAccount::unpack(&self.accounts.debt.as_ref().unwrap().source_ta.data.borrow())?;
    ...
    fn payout_fees(
        ...
        let solauto_fees =
            (balance as f64).mul((self.solauto_fees_bps.solauto as f64).div(10000.0)) as u64;
