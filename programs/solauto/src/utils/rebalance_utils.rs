...
fn get_target_liq_utilization_rate_from_dca(
    ...
    let new_boost_to_bps = (setting_params.boost_to_bps as f64)
        .sub((boost_param_diff as f64).mul(dca_progress)) as u16;
    setting_params.boost_to_bps = new_boost_to_bps;

    let current_rate_diff = current_liq_utilization_rate.sub(target_boost_to_bps);
    let target_rate_bps = (current_liq_utilization_rate as f64)
        .sub((current_rate_diff as f64).mul(dca_progress)) as u16;
...
fn get_target_rate_and_dca_amount(
    ...
    let (target_liq_utilization_rate_bps, amount_to_dca_in) = match dca_instruction {
        true => {
            ...
            let increasing_leverage = !target_boost_to_bps.is_some()
...
mod tests {
    ...
    use crate::{
        types::{
            ...
            shared::{DCASettings, DebtToAddToPosition, SolautoSettingsParameters},
    ...
    fn standard_solauto_position(
        ...
        if active_dca.is_some() && active_dca.as_ref().unwrap().add_to_pos.is_some() {
            ...
            data.debt_ta_balance = (add_to_pos_amt as f64).sub(
                (add_to_pos_amt as f64)
                    .mul((dca.dca_periods_passed as f64).div(dca.target_dca_periods as f64)),
            ) as u64;
        }
    ...
    fn standard_dca_rebalance_validation(
        ...
        if dca_settings.target_boost_to_bps.is_some() {
            ...
            let expected_boost_to_bps = (before_boost_to_bps as f64).sub(
                (before_boost_to_bps.sub(dca_settings.target_boost_to_bps.unwrap() as i16) as f64)
                    .mul(dca_progress),
            ) as u16;
        ...
        if dca_settings.dca_periods_passed == dca_settings.target_dca_periods - 1 {
            ...
            assert!(
                solauto_position
                    ...
                    .unwrap()
                    .dca_periods_passed
                    == dca_settings.dca_periods_passed + 1
            );
        }
    }
    ...
    fn test_dca_rebalance_with_std_validation(
        ...
        let expected_liq_utilization_rate_bps = if dca_settings.target_boost_to_bps.is_some() {
            let curr_utilization_rate_diff = (current_liq_utilization_rate_bps as i16)
                .sub(dca_settings.target_boost_to_bps.unwrap() as i16);
            let dca_progress = dca_progress_percentage(
                dca_settings.target_dca_periods,
                dca_settings.dca_periods_passed,
            );
            (current_liq_utilization_rate_bps as f64)
                .sub((curr_utilization_rate_diff as f64).mul(dca_progress)) as u16
        } else {
            current_liq_utilization_rate_bps
        };
    ...
    fn test_dca_in() {
        ...
        test_dca_rebalance_with_std_validation(
            ...
            DCASettings {
                ...
                add_to_pos: Some(DebtToAddToPosition {
                    base_unit_debt_amount: 60000000,
                    risk_aversion_bps: Some(0),
                }),
            },
            Some(SolautoSettingsParameters {
                boost_to_bps: BOOST_TO_BPS,
        ...
        test_dca_rebalance_with_std_validation(
            ...
            DCASettings {
                unix_start_date: 0,
                dca_interval_seconds: 5,
                dca_periods_passed: 8,
                target_dca_periods: 10,
                target_boost_to_bps: Some(BOOST_TO_BPS),
                add_to_pos: Some(DebtToAddToPosition {
                    base_unit_debt_amount: 500000000,
                    risk_aversion_bps: Some(0),
                }),
            },
            Some(SolautoSettingsParameters {
