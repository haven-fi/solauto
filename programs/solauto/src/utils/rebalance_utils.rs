...
fn get_max_liq_utilization_rate_from_risk_aversion(position_account: &SolautoPosition) -> u16 {
...
fn get_std_target_liq_utilization_rate(
    ...
    {
        let setting_params = position_account.position
            .as_ref()
            .unwrap()
        ...
        Some(direction) =>
            match direction {
                DCADirection::In(_) => {
                    ...
                    if
                        target_rate >
                        get_max_liq_utilization_rate_from_risk_aversion(position_account)
                    {
