...
fn get_additional_amount_to_dca_in(position: &mut PositionData) -> Option<u64> {
    ...
    let percent = dca_progress_percentage(
...
fn get_target_liq_utilization_rate_from_dca(
    ...
    let target_rate_bps =
        (current_liq_utilization_rate as f64).sub((diff as f64).mul(percent)) as u16;
...
mod tests {
    ...
    fn test_rebalance(
        ...
        let settings = if setting_params.is_some() {
            setting_params.unwrap()
        } else {
            default_setting_params()
        };
        let mut solauto_position = standard_solauto_position(settings.clone(), dca_settings);
        let mut obligation_position =
        ...
        let supply_adjustment = expected_debt_adjustment_usd
        ...
        assert!(
            obligation_position.current_liq_utilization_rate_bps() == expected_utilization_rate_bps
        );
        Ok(())
    }
    ...
    fn test_dca_out() {
        test_rebalance(
            5500,
            5500.0.sub(5500.0.mul(dca_progress_percentage(10, 0))) as u16,
            None,
            Some(DCASettings {
                unix_start_date: 0,
