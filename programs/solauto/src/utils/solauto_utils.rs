...
use crate::{
    ...
    types::{
        ...
        shared::{ DeserializedAccount, LendingPlatform, PositionType, RebalanceDirection },
...
pub fn get_owner<'a, 'b>(
    ...
    if solauto_position.data.self_managed.val { signer } else { solauto_position.account_info }
}
...
pub fn create_new_solauto_position<'a>(
    ...
    let data = if update_position_data.setting_params.is_some() {
        ...
        position_data.setting_params = SolautoSettingsParameters::from(
            *update_position_data.setting_params.as_ref().unwrap()
        );
...
pub fn create_or_update_referral_state<'a>(
    ...
    let data: Result<DeserializedAccount<ReferralState>, ProgramError> = if
        account_has_data(referral_state)
    {
        let mut referral_state_account = DeserializedAccount::<ReferralState>
            ::zerocopy(Some(referral_state))?
            .unwrap();
...
pub fn initiate_dca_in_if_necessary<'a, 'b>(
    ...
    spl_token_transfer(token_program, SplTokenTransferArgs {
...
impl SolautoFeesBps {
    ...
    pub fn fetch_fees(&self, rebalance_direction: &RebalanceDirection) -> FeePayout {
        ...
        let mut fee_bps: f64;
        if
            self.target_liq_utilization_rate_bps.is_some() &&
            ...
            rebalance_direction == &RebalanceDirection::Repay
        {
            ...
            let t =
                (self.position_net_worth_usd.ln() - min_size.ln()) /
