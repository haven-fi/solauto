...
pub fn generic_instruction_validation(
    ...
    if accounts.solauto_fees_supply_ta.is_some()
        && !token_account_owned_by(
            solauto_utils::safe_unpack_token_account(accounts.solauto_fees_supply_ta)?
                .as_ref()
                .unwrap(),
            &SOLAUTO_FEES_WALLET,
        )
...
pub fn validate_referral_accounts(
    ...
    if &authority_referral_state.data.authority != referral_state_authority
...
pub fn validate_token_account<'a>(
    ...
    if !solauto_position.data.self_managed.val {
        ...
        if source_ta_data.is_some() && &source_ta_data.as_ref().unwrap().data.mint != mint_key {
            msg!(
                "Incorrect token account {}",
                source_ta_data.unwrap().account_info.key
            );
            return Err(SolautoError::IncorrectAccounts.into());
        }

        if source_ta.is_some()
            && source_ta.unwrap().key
