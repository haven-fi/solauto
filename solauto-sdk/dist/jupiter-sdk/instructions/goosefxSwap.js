"use strict";
/**
 * This code was AUTOGENERATED using the kinobi library.
 * Please DO NOT EDIT THIS FILE, instead use visitors
 * to add features, then rerun kinobi to update it.
 *
 * @see https://github.com/metaplex-foundation/kinobi
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.getGoosefxSwapInstructionDataSerializer = getGoosefxSwapInstructionDataSerializer;
exports.goosefxSwap = goosefxSwap;
const umi_1 = require("@metaplex-foundation/umi");
const serializers_1 = require("@metaplex-foundation/umi/serializers");
const shared_1 = require("../shared");
function getGoosefxSwapInstructionDataSerializer() {
    return (0, serializers_1.mapSerializer)((0, serializers_1.struct)([['discriminator', (0, serializers_1.array)((0, serializers_1.u8)(), { size: 8 })]], { description: 'GoosefxSwapInstructionData' }), (value) => ({
        ...value,
        discriminator: [222, 136, 46, 123, 189, 125, 124, 122],
    }));
}
// Instruction.
function goosefxSwap(context, input) {
    // Program ID.
    const programId = context.programs.getPublicKey('jupiter', 'JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4');
    // Accounts.
    const resolvedAccounts = {
        swapProgram: {
            index: 0,
            isWritable: false,
            value: input.swapProgram ?? null,
        },
        controller: {
            index: 1,
            isWritable: false,
            value: input.controller ?? null,
        },
        pair: { index: 2, isWritable: true, value: input.pair ?? null },
        sslIn: {
            index: 3,
            isWritable: true,
            value: input.sslIn ?? null,
        },
        sslOut: {
            index: 4,
            isWritable: true,
            value: input.sslOut ?? null,
        },
        liabilityVaultIn: {
            index: 5,
            isWritable: true,
            value: input.liabilityVaultIn ?? null,
        },
        swappedLiabilityVaultIn: {
            index: 6,
            isWritable: true,
            value: input.swappedLiabilityVaultIn ?? null,
        },
        liabilityVaultOut: {
            index: 7,
            isWritable: true,
            value: input.liabilityVaultOut ?? null,
        },
        swappedLiabilityVaultOut: {
            index: 8,
            isWritable: true,
            value: input.swappedLiabilityVaultOut ?? null,
        },
        userInAta: {
            index: 9,
            isWritable: true,
            value: input.userInAta ?? null,
        },
        userOutAta: {
            index: 10,
            isWritable: true,
            value: input.userOutAta ?? null,
        },
        feeCollectorAta: {
            index: 11,
            isWritable: true,
            value: input.feeCollectorAta ?? null,
        },
        userWallet: {
            index: 12,
            isWritable: false,
            value: input.userWallet ?? null,
        },
        feeCollector: {
            index: 13,
            isWritable: false,
            value: input.feeCollector ?? null,
        },
        tokenProgram: {
            index: 14,
            isWritable: false,
            value: input.tokenProgram ?? null,
        },
    };
    // Default values.
    if (!resolvedAccounts.tokenProgram.value) {
        resolvedAccounts.tokenProgram.value = context.programs.getPublicKey('splToken', 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA');
        resolvedAccounts.tokenProgram.isWritable = false;
    }
    // Accounts in order.
    const orderedAccounts = Object.values(resolvedAccounts).sort((a, b) => a.index - b.index);
    // Keys and Signers.
    const [keys, signers] = (0, shared_1.getAccountMetasAndSigners)(orderedAccounts, 'programId', programId);
    // Data.
    const data = getGoosefxSwapInstructionDataSerializer().serialize({});
    // Bytes Created On Chain.
    const bytesCreatedOnChain = 0;
    return (0, umi_1.transactionBuilder)([
        { instruction: { keys, programId, data }, signers, bytesCreatedOnChain },
    ]);
}
